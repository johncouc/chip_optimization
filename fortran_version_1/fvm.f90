...
CONTAINS
  ...
  SUBROUTINE fvm_simulate(v, d,DAT,IDAT)
	...
	REAL(8)                                   ::  lengthx, lengthy, lengthz, Q, Ts, p
      ...
      implicit none
        ...
        subroutine ADJOINT(v,d,GRAD,DAT,IDAT)
              ...
              REAL(8):: IDAT(d+1,d+1)
                ...
                integer:: IERR
                integer       :: l,b,a,i, j, ii, jj, m !number of design variables
        	REAL(8)                                   :: Cost, lengthx, lengthy, lengthz, Q, Ts, p
...
!FIELD ADJOINT EQ:
               ...
               CALL solver_v11(DAT(1:d*d,1:d*d),-DAT(1:d*d,d*d+1),d*d,PSI)
...
!! 4th ELEMENT
 ...
 subroutine EV_G(N, X, NEW_X, M, G, DAT, IDAT, IERR)
      implicit none
      integer N, NEW_X, M
      double precision G(M), X(N), ONES(N)
            
      REAL(8) DAT( (floor(sqrt(N*1d0))+1)**2, (floor(sqrt(N*1d0))+1)**2+1),IDAT(floor(sqrt(N*1d0)) + 2,(floor(sqrt(N*1d0))+ 2))
 
       integer IERR

      write(*,*) "ev_g, new_x=",NEW_X
