...
C
      ...
      parameter  (N = 29*29, M = 1, NELE_JAC = N, NELE_HESS = 0)
      parameter  (IDX_STY = 1 )
...
C
      double precision DAT((d)**2,(d)**2+1)
      double precision IDAT(d+1,d+1)
C
...
C
      IERR = IPADDINTOPTION(IPROBLEM, 'max_iter', 180)
      if (IERR.ne.0 ) goto 9990
C
