...
C =============================================================================
C
C                         Topology optimization for cooling device
C
C =============================================================================
C
      PROGRAM main
...
C     Size of the problem (number of variables and equality constraints)
      ...
      PARAMETER  (N =120*120, M = 1, NELE_JAC = N, NELE_HESS = 0)
      PARAMETER  (IDX_STY = 1 )
...
C     Set a string option
     ...
     1'limited-memory')
C      IERR = IPADDSTROPTION(IPROBLEM, 'mu_oracle', 'loqo')
      IF (IERR .NE.0 ) goto 9990
C      IERR = IPADDSTROPTION(IPROBLEM, 'mehrotra_algorithm', 'no')
      IERR = IPADDSTROPTION(IPROBLEM, 'print_timing_statistics','no')
      if (IERR.ne.0 ) goto 9990
C      IERR = IPADDSTROPTION(IPROBLEM, 'mu_oracle', 'probing')
C      if (IERR.ne.0 ) goto 9990


C      IERR = IPADDSTROPTION(IPROBLEM, 'pardiso_order', 'pmetis')
C      IERR = IPADDINTOPTION(IPROBLEM,'pardiso_msglvl',
C     10)

C         select SIMP strategy (works best for p=const=3)

C	  !pvaries: binary: 1 = vary p from 1 to 3, 0 = fix p = 3 

         pvaries = 0
C	  !point1: iteration to begin increasing p

         point1 = 10
C	  !point2: iteration to finish increasing p
